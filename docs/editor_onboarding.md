...
- Note that the EiC may have initiated a scope review. The editor should not start editing until this has completed. Scope reviews now evaluate not just scholarly effort but also development practices (timeline, open development, collaborative effort) and research significance. Editors are able to query the scope (as are reviewers) if they think the EiC should have (but didn't).
